  ...
  class Base
    class << self
      ...
      def select_menu(id, options, placeholder = nil, min_values: nil, max_values: nil, &block)
        raise ArgumentError, "block required" unless block_given?
        real_options = options.map { |option| option.is_a?(Discorb::SelectMenu::Option) ? option : Discorb::SelectMenu::Option.new(*option) }
        menu = Discorb::SelectMenu.new(id, real_options, placeholder: placeholder, min_values: min_values, max_values: max_values)
